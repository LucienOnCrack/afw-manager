...
export async function POST(request: NextRequest) {
  try {
    ...
    const linings = (data ?? []).map((row) => ({
      ...
      CutLengthLining: row.cut_length_fabric ?? false,
    ...
    return NextResponse.json({ linings });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Lining search failed";
    console.error(`[Lining Search] ${message}`);
